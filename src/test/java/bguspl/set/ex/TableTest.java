...
class TableTest {
    ...
    void assertInvariants() {
        // slotToCard[x] == y iff cardToSlot[y] == x
        Integer oneslotToCard = 0;
        Integer onecardToSlot = 0;

        for (int i = 0; i < slotToCard.length; ++i) {
            oneslotToCard = slotToCard[i];
            if(oneslotToCard!=null){
                onecardToSlot = cardToSlot[slotToCard[i]];
                Integer check = i;
                assertEquals(check, onecardToSlot);
            }
        }
    }
    ...
    void isTheSlotFree(){
        ...
        for (int i = 0; i < slotToCard.length; i++) {
            if(i==1 || i==2){
                assertEquals(false, table.isTheSlotFree(i));
            }
            else{
                assertEquals(true, table.isTheSlotFree(i));
            }
        }
    }
    ...
    void getCardInSlot(){
        ...
        for (int i = 0; i < slotToCard.length; i++) {
            if(i==1 || i==2){
                if(i==1)
                assertEquals(3, table.getCardInSlot(i));
                else
                assertEquals(5, table.getCardInSlot(i));
            }
            else{
                assertEquals(null, table.getCardInSlot(i));
            }
        }
    }
